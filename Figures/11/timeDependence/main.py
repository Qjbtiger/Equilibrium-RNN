...
RNN1 = rate_model(N, g, eta, 0.01 if sigma == 0 else sigma, nl, T, dt, dist, scale)   
...
plt.ylabel("$m_x$")
plt.legend()
plt.savefig("./Figures/e4/mx.pdf")

plt.figure(figsize=(5,4), dpi=200)
plt.plot(np.diag(Chix1), label='numerical', lw=4)
plt.plot(np.diag(Chix2), label='DMFT', ls='--', lw=4)
plt.ylabel("$\\chi_x$")
plt.legend()
plt.savefig("./Figures/e4/Chix.pdf")
...
plt.savefig("./Figures/e4/main.pdf")
